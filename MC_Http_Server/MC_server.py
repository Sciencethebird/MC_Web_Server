from flask import Flask, render_template, redirect, url_for, Response, jsonify ,request, stream_with_context
import pandas as pd
import numpy as np
import subprocess
...
class server_console():
    ...
    def send_message(self, message):
        ...
        if self.server_status:
            try:
                #time.sleep(2)
                self.server.stdout.flush()       
                self.server.stdin.write(bytes(message+"\n", "ascii"))
                self.server.stdin.flush()                          
            except Exception as e:
                print(e)
    ...
    def onoff_message(self):
        ...
        if self.server_status == False and self.launching == False:
            ...
            yield '#CD5C5C',"Offline", ">\tPress Start !", "0%", "Server is Currently Offline...", "<li>None</li>", "free"
            return
        ...
        while True:
            ...
            if self.launching:
                ...
                if first:
                    yield '#CD5C5C',"Launching....", ">  Allocating Memory.....", "10%", "Launching server.exe", "<li>None</li>", "free"
                    first = False
                    while self.msg == "":
                        pass
                    ...
                    self.launching = False
                    yield '#4CAF50',"Online"," ".join(self.msg[3:5]), "100%" , self.history, "<li>None</li>", "free"
                    #return
                else:
                    yield '#CD5C5C',"Launching....", out, progress, out,"<li>None</li>", self.page_index
            elif self.server_status:   
                #print(self.history)
                yield '#4CAF50',"Online", out, "100%", self.history, self.player_list_msg, self.page_index
                #self.page_index = "free"
...
def main_page():
    print("\n[Request]: Main Page, {}\n".format(datetime.now()) )
    #MCS.page_index = "Play_Page"
    if request.method =='POST':
        #print(request.values)
        #print('hhhhh')
        
        MCS.send_message(request.values["commands"])
        #MCS.page_index = "message_board"
             
        # if request.values['send']=='送出':
        # return render_template('main.txt',name=request.values['user'])
    #print("cc",MCS.page_index)    
